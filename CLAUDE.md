...
- `data/chair_outlook.json` — hand-curated assessment of the 2026–30 Regional Chair appointment (provincial appointment, applications closed 2026-07-13, appointee announced before Nov 15). Every signal is typed `confirmed | inferred | rumor` with source + date; the dashboard renders it verbatim in the Post-Election Outlook panel with speculative styling. Refresh by re-running the news/web/Reddit sweep and bumping `last_reviewed`; key checkpoints are Jolliffe's post-July-13 statement and which incumbents don't file by Aug 21
...
data/chair_outlook.json             # Hand-curated chair-appointment assessment (typed signals, see above)
