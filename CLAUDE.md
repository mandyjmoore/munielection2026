...
16 of 22 registered (73%), **at risk** — needs 17 by Aug 21 2 p.m. (32 days out). Five unfiled: Scarpitti + Taylor (assessed **likely to run** → outlook's light-green tier), Hackson (no recorded assessment), Vegh + M. Chan (no owner read). Markham's at-large field is 4-for-4 → Jones/Li/Ho/Kristian Chan all "favored — on track for acclamation"; Kristian Chan is projected to take M. Chan's seat as "Registered · alignment unscored" (he has no voting record; editorial assessment pending — a fiscal-alignment override mechanism for unscored challengers is NOT built yet, only `likely_to_run_again_override` exists). Outlook bar: 8/3/2/2/2/1/1 + 3 unknown (Hackson, Vegh, Chair), with a ⅔·15-seat benchmark tick — aligned+neutral currently lands exactly on it.
