...
- `data/manual_overrides.json` — human-verified facts for what scrapers can't reach (primarily **Vaughan** filings) AND editorial assessments: a `likely_to_run_again_override` field ({label, basis, confidence, source:"owner"}) beats the news heuristic in estimate_race_outlook.py and feeds the Post-Election Outlook's light-green "Aligned · likely to run" tier (assessments recorded 2026-07-21). Applied by main.py after every fetch, so they always win. Each entry needs provenance (verified_by / verified_on / source). Lesson from the Del Duca miss (showed "Not registered" for two months after he filed May 1): the news feed can't backfill events older than its collection start, so Vaughan corrections MUST come through this file when the owner checks vaughan.ca/council/elections/candidates
...
16 of 22 registered (73%), **at risk** — needs 17 by Aug 21 2 p.m. Five unfiled: Scarpitti + Taylor (assessed **likely to run** → outlook's light-green tier), Hackson (no recorded assessment), Vegh + M. Chan (no owner read). Markham's at-large field is 4-for-4 → Jones/Li/Ho/Kristian Chan all "favored — on track for acclamation" on tiles, **but the projection does NOT seat a challenger until actual acclamation** (owner correction 2026-07-22: the field can grow until Aug 21, so M. Chan's seat stays in Outcome unknown; K. Chan would claim it — as "Registered · alignment unscored" — only when the estimator says `acclaimed`, i.e. post-close. His fiscal-alignment editorial assessment is pending; the challenger-alignment override mechanism is NOT built yet, only `likely_to_run_again_override` exists). Outlook bar: 8/2/2/3/2/1 + 4 unknown (M. Chan, Hackson, Vegh, Chair) of 22, ⅔·15-seat benchmark counted CONSERVATIVELY (owner decision 2026-07-22): only the aligned family (strongly aligned + aligned, registered or likely-to-run) counts as "likely supportive" — neutral is no measured lean, not a reliable vote (esp. under a strong-chair dynamic). A dark supportive-extent line along the bar's bottom stops at 12 today, 3 short of the tick; label reads "⅔ · 15 seats — likely supportive: 12 (−3)".

## The page's visual-semantics system (owner-driven, 2026-07-21 — keep it consistent)
...
- **Verification loop that works (2026-07-21):** headless jsdom test against a Bash-launched local server for logic, then the owner's "YR 2026 Election" Safari Dock web app for visuals (computer-use grant, full tier: Cmd+Alt+R to hard-reload past the ~10-min Pages HTML cache, screenshot, zoom). Her window is ~1450px — narrower than dev defaults; test layout at that width.
- **Feedback form** (header button) relays via FormSubmit to the configured inbox — activated and tested 2026-07-21. **GoatCounter analytics** live (wordi.goatcounter.com, host-prefixed paths; "visits" = daily uniques).
- **Scheduled**: `vaughan-candidate-check` every 2h Mon–Fri business hours (needs desktop app + Chrome open); one-shot `kristian-chan-alignment-assessment` reminder fires 2026-07-22 09:00 — owner will provide a fiscal-alignment read on Kristian Chan; the challenger-alignment override mechanism must be built to record it. Standing backlog: alignment assessments for ALL challengers.
