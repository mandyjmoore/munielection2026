...
16 of 22 registered (73%), **at risk** — needs 17 by Aug 21 2 p.m. Five unfiled: Scarpitti + Taylor (assessed **likely to run** → outlook's light-green tier), Hackson (no recorded assessment), Vegh + M. Chan (no owner read). Markham's at-large field is 4-for-4 → Jones/Li/Ho/Kristian Chan all "favored — on track for acclamation" on tiles, **but the projection does NOT seat a challenger until actual acclamation** (owner correction 2026-07-22: the field can grow until Aug 21, so M. Chan's seat stays in Outcome unknown; K. Chan would claim it — as "Registered · alignment unscored" — only when the estimator says `acclaimed`, i.e. post-close. His fiscal-alignment editorial assessment is pending; the challenger-alignment override mechanism is NOT built yet, only `likely_to_run_again_override` exists). Outlook bar: 8/2/2/3/2/1 + 4 unknown (M. Chan, Hackson, Vegh, Chair) of 22, ⅔·15-seat benchmark tick — aligned+neutral lands exactly on it (Jackson's guard-driven move from Aligned to Neutral shifted mass within the left-of-line side, not across it).
